...
namespace {
  struct DiscoPoPOpenMP : public FunctionPass {
    ...
    int64_t mainRegionsUID;
    int64_t extFuncRegionsUID;
 ...
 // 
  ...
  if(regName.startswith(".omp"))
  {
    ...
    currrRegionId = stoi(cast<MDString>(currentRegion->getMetadata("omp.region.ID")->getOperand(0))->getString());
     Tag= cast<MDString>(currentRegion->getMetadata("omp.region.ID")->getOperand(0));
  }
  else if(regName.equals("main"))//Master Thread works in main
  { 
    ...
    Tag = cast<MDString>(currentRegion->getMetadata("main.region.ID")->getOperand(0));
  }
  else
  {
    if (currentRegion->getMetadata("externalFunction.region.ID")==NULL)
    {
      Vals.push_back(MDHelper.createString(to_string(ompRegionsUID++)));
      MDNode *N = MDNode::get(currentRegion->getContext(), Vals);
      currentRegion->setMetadata("externalFunction.region.ID", N); 
    }
    currrRegionId = stoi(cast<MDString>(currentRegion->getMetadata("externalFunction.region.ID")->getOperand(0))->getString());
    Tag = cast<MDString>(currentRegion->getMetadata("externalFunction.region.ID")->getOperand(0));
  }
