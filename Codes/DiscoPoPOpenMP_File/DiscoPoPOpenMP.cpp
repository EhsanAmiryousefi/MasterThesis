 ...
 *
 *
 *  Created by Mohammad Norouzi and Zia Ul-Huda on 18/05/2016.
 *  
 *  
 *
 */

//#define DEBUG_TYPE "DiscoPoPOpenMP"
...
namespace {
  ...
  static cl::opt<std::string> InputFilename("input1", cl::desc("<input file>"), cl::Optional);
  static cl::opt<std::string> FileMapping("filemapping1", cl::desc("<filemapping file>"), cl::Optional);
  ...
  struct DiscoPoPOpenMP : public ModulePass {
    ...
    //map<string, int> signature; 
     ...
     // Callbacks to run-time library
      Function *DPOMPFinalize, *DPOMPInitialize;
      Function *DPOMPRead, *DPOMPWrite;
      Function *DPOMPCallBefore;
      Function *DPOMPCallAfter;
...
string DiscoPoPOpenMP::getFunctionName(Instruction *instruction){
...
// string split
vector<string> DiscoPoPOpenMP::split(const string &s, const char delim) {
  stringstream ss(s);
  string tmp;
  vector<string> out;
...
void DiscoPoPOpenMP::insertInitializeInst(Function &F){
  ...
  Value *vName = builder.CreateGlobalStringPtr(StringRef(allFunctionIndices.c_str()), ".str");
    ...
    // We always want to insert __DPOMPInitialize at the beginning
    ...
    for (BasicBlock::iterator BI = entryBB.begin(), EI = entryBB.end(); BI != EI; ++BI) {
        ...
        if (lid > 0 && !isa<PHINode>(BI)) {
            ...
            //IRB.CreateCall2(DPOMPInitialize);
            //CallInst::Create(DPOMPInitialize, args, "")->insertAfter(BI);  
            CallInst::Create(DPOMPInitialize, args, "", BI);
            break;
        }
    }
...
void DiscoPoPOpenMP::instrumentLoadInst(Instruction *toInstrument, int pidIndex){
    vector<Value*> args;
...
void DiscoPoPOpenMP::instrumentCallInst(Instruction *toInstrument, int index, int lastCall){
  vector<Value*> args;
  ...
  //Value *vName = builder.CreateGlobalStringPtr(StringRef(fName.c_str()), ".str");
    ...
    //it creates a call "DPOMPCallBefore" and inserts it before the toInstrument instruction
    CallInst::Create(DPOMPCallBefore, args, "", toInstrument);
    ...
    //it first creates a call "DPOMPCallAfter" and then inserts it after the toInstrument instruction
    CallInst::Create(DPOMPCallAfter, args, "")->insertAfter(toInstrument); 

}

void DiscoPoPOpenMP::getAnalysisUsage(AnalysisUsage &Info) const {
    Info.addRequired<LoopInfo>();
}

int DiscoPoPOpenMP::isLastCall(LID line, string fName, int index){
...
INITIALIZE_PASS(DiscoPoPOpenMP, "DiscoPoPOpenMP", "DiscoPoPOpenMP: determine computation units instances that can run in parallel.", false, false)

ModulePass *llvm::createDiscoPoPOpenMPPass()
{
...
const char* DiscoPoPOpenMP::getPassName() const
{
  return "DiscoPoPOpenMP";
}