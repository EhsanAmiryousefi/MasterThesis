...
//#define DEBUG_TYPE "CUInstantiation"
...
void CUInstantiation::determineFileID(Function &F) {
	fileID = 0;
...
int CUInstantiation::getLID(Instruction* BI)
{
...
string CUInstantiation::decodeLID(LID lid) {
	if (lid == 0)
		return "*";
...
void CUInstantiation::instrumentLoadInst(Instruction *toInstrument, int pidIndex, string writeLine){
    vector<Value*> args;
...
INITIALIZE_PASS(CUInstantiation, "CUInstantiation", "CUInstantiation: determine computation units instances that can run in parallel.", false, false)

ModulePass *llvm::createCUInstantiationPass()
{
...
const char* CUInstantiation::getPassName() const
{
