...
for (dirpath, dirnames, filenames) in os.walk(os.getcwd()):
    for patternFile in filenames:
        if patternFile.endswith('.txt'):
            print patternFile
            path = os.path.join(os.getcwd(), 'comms/ehsan', patternFile)
            matData = np.genfromtxt(path, delimiter=' ', dtype=np.int, autostrip=True)
...
#            plt.title('Master/Worker')
            ...
            fig.savefig(os.path.join(os.getcwd(), 'comms/ehsan',os.path.splitext(patternFile)[0]+'.pdf'))
